...
gameinput="""Game 1: 4 blue, 16 green, 2 red; 5 red, 11 blue, 16 green; 9 green, 11 blue; 10 blue, 6 green, 4 red
...
class Set:
    ...
    def __init__(self, string) -> None:
        ...
        for color in string.split(","): 
            cnt, clr = color.strip().split(" ")
            self.colors[clr] = int(cnt)
...
class Game:
    ...
    def __init__(self, string):
        ...
        self.sets = [Set(set) for set in string.split(":")[1].split(";")]

    def is_possible(self, limits):
        for set in self.sets:
            for color, cnt in set.colors.items():
                if cnt > limits[color]:
                    return False
        return True
    ...
    def get_prod(self):
        ...
        for set in self.sets:
            for color, cnt in set.colors.items():
                if not color in numbers.keys():
                    numbers[color] = cnt
                elif numbers[color] < cnt:
                    numbers[color] = cnt
...
if __name__ == "__main__":
    # part1
    possible_game_ids = []
    for _game in example.splitlines():
        game = Game(_game)
        if game.is_possible(limits={"red": 12, "green": 13, "blue": 14}):
            possible_game_ids.append(game.id)
    assert sum(possible_game_ids) == 8

    possible_game_ids = []
    for _game in gameinput.splitlines():
        game = Game(_game)
        if game.is_possible(limits={"red": 12, "green": 13, "blue": 14}):
            possible_game_ids.append(game.id)
    ...
    for _game in example.splitlines():
        game = Game(_game)
        products.append(game.get_prod())
    assert sum(products) == 2286
